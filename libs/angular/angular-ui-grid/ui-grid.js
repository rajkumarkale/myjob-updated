    ...
    compile: function() {
      return {
        pre: function($scope, $elm, $attrs, uiGridCtrl) {
          ...
          // If the grid controllers is present, use it to get the compiled cell template function
          if (uiGridCtrl && $scope.col.compiledElementFn) {
             compileTemplate();
          }
          // No controllers, compile the element manually (for unit tests)
          else {
            if ( uiGridCtrl && !$scope.col.compiledElementFn ){
 ...
 */
  ...
  var service = {
    ...
    initialize: function( $scope, uiGridCtrl ){
      ...
      // Store a reference to this link/controllers in the main uiGrid controllers
          ...
          pre: function ($scope, $elm, $attrs, controllers) {
            ...
            gridUtil.getTemplate(footerTemplate)
              .then(function (contents) {
                ...
                if (containerCtrl) {
                  // Inject a reference to the footer viewport (if it exists) into the grid controllers for use in the horizontal scroll handler below
          ...
          pre: function ($scope, $elm, $attrs, controllers) {
            ...
            gridUtil.getTemplate(headerTemplate)
              .then(function (contents) {
                ...
                if (containerCtrl) {
                  // Inject a reference to the header viewport (if it exists) into the grid controllers for use in the horizontal scroll handler below
        ...
        post: function ($scope, $elm, $attrs, controllers) {
          ...
          $scope.itemAction = function($event,title) {
            ...
            if (typeof($scope.action) === 'function') {
              ...
              // Add the grid to the function call context if the uiGrid controllers is present
              if (typeof(uiGridCtrl) !== 'undefined' && uiGridCtrl) {
                context.grid = uiGridCtrl.grid;
              }
...
(function () {
  ...
  angular.module('ui.grid').controller('uiGridController', ['$scope', '$element', '$attrs', 'gridUtil', '$q', 'uiGridConstants',
    ...
    function ($scope, $elm, $attrs, gridUtil, $q, uiGridConstants,
              $templateCache, gridClassFactory, $timeout, $parse, $compile) {
      // gridUtil.logDebug('ui-grid controllers');
  ...
  return {
    ...
    compile: function () {
      return {
        post: function ($scope, $elm, $attrs, uiGridCtrl) {
          var grid = uiGridCtrl.grid;
          // Initialize scrollbars (TODO: move to controllers??)
  ...
  Grid.prototype.buildStyles = function buildStyles() {
    ...
    self.styleComputations
      ...
      .forEach(function (compInfo) {
        ...
        // style builders that used it, so removed it as part of moving to grid from controllers
